...
| Logical project    | An explicitly named body of work under a project, with optional existing checkouts in multiple repositories. Registration records ownership metadata, not execution authority. |
| Repository binding | A logical project's adopted checkout in a local Git repository, identified by its canonical common Git directory. Separate clones have separate local identities.              |
